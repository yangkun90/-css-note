...
6. 垂直居中的。vertical-align 这个只是音像行内元素或者table-cell。 块内部文字居中单行，使用line-height和高度相同可以文本居中。或者给块设置上下padding，内容自动适应。
   ...
   1. 最开始做图文布局的。文字会自动环绕浮动的元素。比如环绕图片
   
   ![2](/2.png)
   
   2. 浮动的本质就是让块可以在一行显示。
   
   3. 浮动的特点。第一被浮动的块元素可以看做类似行内块，可以设置宽高，如果没有设置宽高默认是内容的宽度。第二浮动脱离了文档流。文档流就是元素本身占据的位置。如果下面有文档流，会自动上来，在浮动元素下面。
   
      第三浮动不会有文档流的bug。比如上边距塌陷合并，子父上边距塌陷都没有。这个很不错。
   
   4. 浮动陷阱。
   
      ![3](/3.png)	
   
      3号我们想在1下面，但是因为2的高度小，3会置顶上去。浮动总是紧贴。
   
   5. 一行的元素最好都浮动。否者浮动会影响到没有浮动的元素，对非浮动元素的行内元素产生影响。
   6. clear：both  清楚浮动。
   
   ## flexbox布局
   
   1. 弹性布局。displa:flex  弹性布局的好处就是可以设置水平纵向的位置。解决水平垂直，横向居中。非常简单。
   
   2. 浮动的子元素。如果设置margin-方向:auto 。Flexbox允许使用margin:auto来填充弹性子元素之间的可用空间。
   
      ![4](/4.png)	
   
      比如about 设置了margin-left:auto 来压向right面
   
   3. 子元素的三个属性。flex-basis 设置元素大小的基准值。可以多种单位，包括px、em、百分比
   
      ![5](/5.png)	
   
      flex-grow 增长因子 比如flex-basic 设置了三个20%，剩余40%没有分配。资格时候flex-grow把剩余的分配给。两个同时flex-grow值都一样1:1 表示都是增长20%。如下图
   
      ![7](/7.png)	
   
      flex-shrink 收缩子元素防止溢出。3个40% 溢出20%。设置了flex-shringk 表示溢出隐藏。
